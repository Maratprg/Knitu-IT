...
Основная  рабочая последовательность команд реализующих главное действие которое описано
в самом начале этой статьи. А именно скорми локальные изменения в локальный репозиторий.
    git add *.*
...
Проверка статуса и определение головы Head проекта.
    ...
    git log      - проверка последних изменений или просмотр истории коммитов.
...
Ps:
Особвенные команды
git branch -M <name>   - изменение названия текущей ветки на которую указывает Голова.
git log   - объевление этой команды может быть расширено
                    большим количеством индексов для детализации истории комитов (см мануал)
Помните git add Name индексирование(I)? Так вот git reset Name это разиндексирование(М).
git restore <file> специальная команда для изменения последних изменений в файле. Это
    довольно таки опсаная команда которая просто откатит файл а обратно  нельзя.

git remote -v Ремоут с ключем выведет два адреса отгрузочный и загрузочный на сервер
Для отгрузки и загрузки содержимого с сервера но для конкретной ветви. Если удаленных
серверов много такие отгрузочно разгрузочные адреса будут выведены для каждого участника команды.
Что означает что можно легко отгрузиться от любого из участников команды.
origin	https://github.com/schacon/ticgit (fetch)
origin	https://github.com/schacon/ticgit (push)
    в конкретно таком случае origin принимает еще и имя проектанта и git fetch <Name> выгрузит именно тот.

После фетчинга текущая папка остается прежний. Если вы действительно хотите смаржить ее то
используйте git merge <branch>

Если же вы просто клонируете проект способом get clone <url>
имя origin остается таким по умолчанию а команды фетчинга устанавливаются сами так что
вы получаете у себя полностью валидный на данную дату проект.  
